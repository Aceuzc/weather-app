...
let API_KEY = readStorage(localStorage, API_KEY_STORAGE, '');

function hasClientApiKey() {
	return Boolean(API_KEY && API_KEY !== 'YOUR_OPENWEATHERMAP_API_KEY');
}

function saveClientApiKey(key) {
	API_KEY = key.trim();
	return writeStorage(localStorage, API_KEY_STORAGE, API_KEY);
}

function runAfterCriticalWork(callback, timeout = 1400) {
...
function getFavorites(){ try { return JSON.parse(readStorage(localStorage, FAVORITES_KEY, '[]')); } catch { return []; } }
function getRecent()   { try { return JSON.parse(readStorage(localStorage, RECENT_KEY, '[]'));    } catch { return []; } }

function addToRecent(cityName){
	let recent = getRecent();
	recent = [cityName, ...recent.filter(c => c.toLowerCase() !== cityName.toLowerCase())].slice(0, 6);
	writeStorage(localStorage, RECENT_KEY, JSON.stringify(recent));
	renderQuickAccess();
}
...
function toggleFavorite(){
	...
	writeStorage(localStorage, FAVORITES_KEY, JSON.stringify(favs));
	updateFavoriteBtn();
	renderQuickAccess();
}
...
function updateFavoriteBtn(){
	...
	btn.innerHTML = uiIcon(isFav ? 'starFilled' : 'star', 'ui-icon', isFav ? 'Favorite city' : 'Add favorite');
...
function renderQuickAccess(){
	...
	if(favs.length > 0){
		...
		favChips.innerHTML = favs.map(city =>
			`<button class="quick-chip" data-city="${escapeHTML(city)}">${escapeHTML(city)}</button>`
		).join('');
...
async function fetchUVIndex(lat, lon){
	try {
		const data = await fetchWeather('uvi', { lat, lon }, { silent: true });
...
function renderUVIndex(uv){
	...
	el.innerHTML = `
		<div class="uv-top">
			<span class="uv-icon">${uiIcon('sun', 'ui-icon', 'UV index')}</span>
			<div>
		...
		<div class="uv-bar-track">
			<div class="uv-bar-fill" style="transform:scaleX(${pct / 100});background:${color}"></div>
		</div>
	`;
}
// ─────────────────────────────────────────────────────────────────────────────
...
function mountQueuedMapFrame() {
	...
	mapFrameLoaded = true;
	mapFrameQueued = false;
}

function scheduleMapFrameMount() {
	if (mapFrameLoaded || mapFrameQueued || !elements.weatherMap?.dataset.src) return;
	mapFrameQueued = true;
	runAfterCriticalWork(mountQueuedMapFrame, 2200);
}

function queueMapFrameSrc(src) {
	const frame = elements.weatherMap;
	if (!frame) return;
...
const ICON_PATHS = {
	...
	camera: '<path d="M14.5 6 13 4H9L7.5 6H5a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2h-4.5Z"/><circle cx="12" cy="12.5" r="3.5"/>',
	check: '<path d="m5 12 4 4L19 6"/>',
	chevronDown: '<path d="m7 10 5 5 5-5"/>',
	chevronUp: '<path d="m7 14 5-5 5 5"/>',
	cloud: '<path d="M17.5 18H8a5 5 0 1 1 1.6-9.73A6 6 0 0 1 21 11.5 3.5 3.5 0 0 1 17.5 18Z"/>',
	cloudSun: '<path d="M16.5 18H8a4 4 0 1 1 1.3-7.78A5.3 5.3 0 0 1 20 12.8 3.2 3.2 0 0 1 16.5 18Z"/><circle cx="15.5" cy="6.5" r="3"/><path d="M15.5 1.5v2M21.5 6.5h1M19.7 2.3l-1.3 1.3"/>',
	coffee: '<path d="M5 8h11v5a5 5 0 0 1-5 5H10a5 5 0 0 1-5-5V8Z"/><path d="M16 10h1.5a2.5 2.5 0 0 1 0 5H16M6 3v2M10 3v2M14 3v2"/>',
	droplet: '<path d="M12 3s6 6.4 6 11a6 6 0 0 1-12 0c0-4.6 6-11 6-11Z"/>',
	drizzle: '<path d="M17.5 17H8a4.5 4.5 0 1 1 1.4-8.78A5.6 5.6 0 0 1 21 11.4 3.4 3.4 0 0 1 17.5 17Z"/><path d="M9 20v1M13 19.5v1M17 20v1"/>',
	error: '<circle cx="12" cy="12" r="9"/><path d="M12 8v5M12 16h.01"/>',
	eye: '<path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.5"/>',
	gauge: '<path d="M4 14a8 8 0 1 1 16 0"/><path d="M12 14l4-5"/><path d="M8 19h8"/>',
	...
	heartFilled: '<path d="M20.8 8.6c0 5.2-8.8 10.4-8.8 10.4S3.2 13.8 3.2 8.6A4.6 4.6 0 0 1 12 6.3a4.6 4.6 0 0 1 8.8 2.3Z" fill="currentColor"/>',
	...
	mist: '<path d="M4 8h16M3 12h13M7 16h14"/>',
	pressure: '<path d="M4 14a8 8 0 1 1 16 0"/><path d="m12 14 4-4"/><path d="M7 18h10"/>',
	rain: '<path d="M17.5 17H8a4.5 4.5 0 1 1 1.4-8.78A5.6 5.6 0 0 1 21 11.4 3.4 3.4 0 0 1 17.5 17Z"/><path d="M8.5 20.5 8 22M12.5 20l-.5 2M16.5 20.5 16 22"/>',
	refresh: '<path d="M20 6v5h-5"/><path d="M4 18v-5h5"/><path d="M19 11A7 7 0 0 0 7.8 5.4L4 9M5 13a7 7 0 0 0 11.2 5.6L20 15"/>',
	scale: '<path d="M12 3v18M5 7h14"/><path d="m6 7-3 6h6L6 7ZM18 7l-3 6h6l-3-6Z"/>',
	shirt: '<path d="M8 4 5 6l-2 4 4 2 1-2v10h8V10l1 2 4-2-2-4-3-2a4 4 0 0 1-8 0Z"/>',
	snow: '<path d="M17.5 17H8a4.5 4.5 0 1 1 1.4-8.78A5.6 5.6 0 0 1 21 11.4 3.4 3.4 0 0 1 17.5 17Z"/><path d="M12 19v3M10.5 20.5h3M16 19l1.5 2M17.8 19.6l-2.6 1.4"/>',
	star: '<path d="m12 3 2.7 5.5 6.1.9-4.4 4.3 1 6.1-5.4-2.9-5.4 2.9 1-6.1-4.4-4.3 6.1-.9L12 3Z"/>',
	starFilled: '<path d="m12 3 2.7 5.5 6.1.9-4.4 4.3 1 6.1-5.4-2.9-5.4 2.9 1-6.1-4.4-4.3 6.1-.9L12 3Z" fill="currentColor"/>',
	storm: '<path d="M17.5 17H8a4.5 4.5 0 1 1 1.4-8.78A5.6 5.6 0 0 1 21 11.4 3.4 3.4 0 0 1 17.5 17Z"/><path d="m13 12-2 4h3l-2 5"/>',
	sun: '<circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M4.93 19.07l1.41-1.41M17.66 6.34l1.41-1.41"/>',
	target: '<circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="5"/><circle cx="12" cy="12" r="1"/>',
	thermometer: '<path d="M14 14.8V5a2 2 0 0 0-4 0v9.8a4 4 0 1 0 4 0Z"/><path d="M12 8v7"/>',
	umbrella: '<path d="M3 12a9 9 0 0 1 18 0H3Z"/><path d="M12 12v6a3 3 0 0 0 6 0"/>',
	walk: '<circle cx="13" cy="4" r="2"/><path d="M10 21l2-6-3-3 2-5 4 3 3 1M13 15l4 6M8 12l-3 3"/>',
	warning: '<path d="m12 3 10 18H2L12 3Z"/><path d="M12 9v5M12 17h.01"/>',
	wind: '<path d="M3 8h12a3 3 0 1 0-3-3"/><path d="M4 13h16"/><path d="M3 18h11a3 3 0 1 1-3 3"/>',
...
function uiIcon(name, className = 'ui-icon', label = '') {
	const path = ICON_PATHS[name] || ICON_PATHS.cloudSun;
	const aria = label ? `role="img" aria-label="${escapeHTML(label)}"` : 'aria-hidden="true"';
	return `<svg class="${className}" ${aria} viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">${path}</svg>`;
}
...
function weatherIcon(condition, className = 'weather-svg') {
	return uiIcon(getWeatherIconName(condition), className, condition || 'Weather');
}

function drawWeatherMark(ctx, condition, x, y, size) {
	const type = getWeatherIconName(condition);
	const cloudTypes = new Set(['cloudSun', 'cloud', 'rain', 'drizzle', 'storm', 'snow']);
	const sunTypes = new Set(['cloudSun', 'sun']);
	const mistTypes = new Set(['mist', 'wind']);
	const s = size;
	...
	ctx.fillStyle = 'rgba(134,182,232,0.12)';

	if (sunTypes.has(type)) {
		ctx.beginPath();
		ctx.arc(s * 0.35, s * 0.28, s * 0.16, 0, Math.PI * 2);
		ctx.stroke();
		for (let i = 0; i < 8; i++) {
	...
	if (cloudTypes.has(type)) {
		ctx.beginPath();
		ctx.moveTo(s * 0.2, s * 0.65);
		ctx.bezierCurveTo(s * 0.08, s * 0.65, s * 0.08, s * 0.43, s * 0.27, s * 0.43);
		ctx.bezierCurveTo(s * 0.32, s * 0.27, s * 0.55, s * 0.25, s * 0.64, s * 0.43);
		ctx.bezierCurveTo(s * 0.82, s * 0.43, s * 0.9, s * 0.65, s * 0.72, s * 0.65);
		ctx.closePath();
		ctx.fill();
		ctx.stroke();
	}
	...
	if (type === 'rain' || type === 'drizzle') {
		const drops = type === 'rain' ? [0.3, 0.5, 0.7] : [0.38, 0.58];
		drops.forEach((p, i) => {
			ctx.beginPath();
			ctx.moveTo(s * p, s * 0.78);
			ctx.lineTo(s * (p - 0.02), s * (0.92 - i * 0.01));
			ctx.stroke();
		});
	}
	...
	if (type === 'storm') {
		ctx.beginPath();
		ctx.moveTo(s * 0.52, s * 0.68);
		ctx.lineTo(s * 0.44, s * 0.86);
		ctx.lineTo(s * 0.57, s * 0.84);
		ctx.lineTo(s * 0.49, s);
		ctx.stroke();
	}

	if (mistTypes.has(type)) {
		for (let i = 0; i < 3; i++) {
			ctx.beginPath();
			ctx.moveTo(s * 0.1, s * (0.35 + i * 0.18));
			ctx.lineTo(s * (0.86 - i * 0.12), s * (0.35 + i * 0.18));
			ctx.stroke();
		}
	}
...
class WeatherRequestError extends Error {
	constructor(message, options = {}) {
		...
		this.kind = options.kind || 'weather';
		this.details = options.details || '';
	}
}

function buildWeatherProxyUrl(endpoint, params = {}) {
	const base = window.location.origin && window.location.origin !== 'null'
		? window.location.origin
		: window.location.href;
	const url = new URL(WEATHER_PROXY_PATH, base);
...
function buildOpenWeatherUrl(endpoint, params = {}) {
	...
	if(!url.searchParams.has('units') && endpoint !== 'uvi') {
		url.searchParams.set('units', 'metric');
	}
	return url.toString();
}

function cleanApiMessage(text) {
	if(!text) return '';
	try {
...
function userFacingWeatherError(status, text) {
	const detail = cleanApiMessage(text);
	if(status === 400) return detail || 'The weather request was not valid.';
	if(status === 401 || status === 403) return 'Weather access is not authorized. Check the configured API key.';
	if(status === 404) return detail || 'No weather data was found for that city.';
	if(status === 429) return 'The weather service is rate limited. Try again in a minute.';
	if(status >= 500) return 'The weather service is temporarily unavailable.';
	return detail || 'Weather data could not be loaded.';
}
...
async function fetchJSON(url){
	...
	try {
		...
		throw new WeatherRequestError('Network connection failed. Check your connection and try again.', {
			kind: 'network',
			details: err.message
		});
	}
	...
	if(!res.ok) {
		...
		throw new WeatherRequestError(userFacingWeatherError(res.status, text), {
			status: res.status,
			details: text
		});
	}
...
async function fetchWeather(endpoint, params = {}, options = {}) {
	try {
		return await fetchJSON(buildWeatherProxyUrl(endpoint, params));
	} catch (proxyErr) {
		if(hasClientApiKey()) {
			return fetchJSON(buildOpenWeatherUrl(endpoint, params));
		}
		if(options.silent) throw proxyErr;
		const proxyLooksMissing = proxyErr.status === 404 && /not found/i.test(proxyErr.details || '') && !/city/i.test(proxyErr.details || '');
		if(!proxyLooksMissing) throw proxyErr;
		const message = proxyLooksMissing
			? 'Weather service is not configured for this environment.'
			: proxyErr.message;
		throw new WeatherRequestError(message, {
			status: proxyErr.status,
			kind: 'config',
			details: proxyErr.details || proxyErr.message
		});
	}
}
...
function showWeatherStatus({ title, message, actionLabel, onAction, allowLocalKey = false }) {
	const el = elements.weatherStatus;
	if(!el) return;
	el.textContent = '';
	el.hidden = false;

	const body = document.createElement('div');
	body.className = 'weather-status-body';

	const titleEl = document.createElement('div');
	titleEl.className = 'weather-status-title';
	titleEl.textContent = title;
	body.appendChild(titleEl);

	const textEl = document.createElement('div');
	textEl.className = 'weather-status-text';
	textEl.textContent = message;
	body.appendChild(textEl);
	el.appendChild(body);

	const actions = document.createElement('div');
	actions.className = 'weather-status-actions';

	if(actionLabel && typeof onAction === 'function') {
		const retry = document.createElement('button');
		retry.type = 'button';
		retry.className = 'weather-status-btn';
		retry.textContent = actionLabel;
		retry.addEventListener('click', onAction);
		actions.appendChild(retry);
	}

	if(allowLocalKey) {
		const form = document.createElement('form');
		form.className = 'weather-key-form';
		const label = document.createElement('label');
		label.className = 'sr-only';
		label.setAttribute('for', 'weather-api-key-input');
		label.textContent = 'OpenWeatherMap API key';

		const input = document.createElement('input');
		input.id = 'weather-api-key-input';
		input.type = 'password';
		input.autocomplete = 'off';
		input.placeholder = 'OpenWeatherMap API key';
		input.required = true;

		const save = document.createElement('button');
		save.type = 'submit';
		save.className = 'weather-status-btn weather-status-btn--primary';
		save.textContent = 'Use local key';

		form.append(label, input, save);
		form.addEventListener('submit', event => {
			event.preventDefault();
			if(!input.value.trim()) return;
			saveClientApiKey(input.value);
			clearWeatherStatus();
...
function showWeatherError(err, retry) {
	const isConfig = err instanceof WeatherRequestError && err.kind === 'config';
	const canUseLocalKey = !hasClientApiKey() && (isConfig || [401, 403, 500].includes(err.status));
	showWeatherStatus({
		title: canUseLocalKey ? 'Weather source needs setup' : 'Weather data unavailable',
		message: err.message || 'The weather request failed.',
		actionLabel: 'Try again',
		onAction: retry,
		allowLocalKey: canUseLocalKey
	});
}
...
async function generateAISummary(cur, list){
	...
	const prompt = [
		...
		'Keep it under 45 words, plain text only, no bullet points, no quotes.',
...
function renderCurrent(d){
	...
	elements.weatherIcon.innerHTML = weatherIcon(d.weather[0].main, 'weather-svg weather-svg--large');
...
function updateMap(lat, lon){
	...
	queueMapFrameSrc(`https://www.openstreetmap.org/export/embed.html?bbox=${bbox}&layer=mapnik&marker=${lat},${lon}`);
	elements.weatherMap.title = `Weather map centered at ${lat.toFixed(3)}, ${lon.toFixed(3)}`;
}
...
function renderForecast7Days(list, maxDays){
	...
	days.forEach((day, index) => {
		...
		const icon = weatherIcon(mainCond, 'weather-svg forecast-svg');
		...
		el.innerHTML = `
			...
			<div class="fpop-bar" title="${popPct}% chance of rain">
				<div class="fpop-fill" style="transform:scaleX(${popPct / 100});background:${popColor}"></div>
			</div>
...
elements.forecastTabs.addEventListener('click', (e) => {
	...
	// Update active state
	elements.forecastTabs.querySelectorAll('.tab-btn').forEach(b => {
...
elements.summaryTabs.addEventListener('click', (e) => {
	...
	elements.summaryTabs.querySelectorAll('.tab-btn').forEach(b => {
		b.classList.remove('active');
...
function renderHourlyList(list){
	...
	entries.forEach(item => {
		...
		const icon = weatherIcon(item.weather[0].main, 'weather-svg hourly-svg');
		...
		div.innerHTML = `
			...
			<span class="hourly-wind">${uiIcon('wind', 'ui-icon', 'Wind')} ${wind}</span>
		`;
		el.appendChild(div);
	});
}
...
function renderMoreDetails(cur, list){
	...
	const details = [
		{ icon: 'thermometer', label: 'Feels Like', value: `${toDisplay(cur.main.feels_like)}${unitLabel()}`, sub: 'Current sensation' },
		{ icon: 'droplet', label: 'Avg Humidity', value: `${avgHumidity}%`, sub: 'Next 24h average' },
		{ icon: 'wind', label: 'Max Wind', value: `${maxWind} m/s`, sub: 'Next 24h peak' },
		{ icon: 'pressure', label: 'Pressure', value: `${avgPressure} hPa`, sub: 'Avg atmospheric' },
		{ icon: 'chevronUp', label: 'High Temp', value: `${toDisplay(maxTemp)}${unitLabel()}`, sub: 'Next 24h maximum' },
		{ icon: 'chevronDown', label: 'Low Temp', value: `${toDisplay(minTemp)}${unitLabel()}`, sub: 'Next 24h minimum' },
		{ icon: 'cloud', label: 'Cloud Cover', value: `${avgClouds}%`, sub: 'Avg coverage' },
		{ icon: 'eye', label: 'Visibility', value: `${(cur.visibility/1000).toFixed(1)} km`, sub: 'Current' },
	];
	...
	details.forEach(d => {
		...
		card.innerHTML = `
			<div class="detail-icon">${uiIcon(d.icon, 'ui-icon', d.label)}</div>
...
function renderSummaryChart(list){
	...
	canvas.setAttribute(
		'aria-label',
		`Temperature chart for the next 24 hours. Range ${formatTemp(Math.min(...temps))} to ${formatTemp(Math.max(...temps))}.`
	);
}
...
let popularCitiesRefreshQueued = false;

function renderPopularCities(results = popularCitiesCache) {
	...
	results.forEach(({ city, data }) => {
		...
		if(data){
			const icon = weatherIcon(data.weather[0].main, 'weather-svg city-svg');
			const temp = formatTemp(data.main.temp);
			const desc = data.weather[0].description.replace(/\b\w/g, c => c.toUpperCase());
			div.innerHTML = `
				<span class="city-name">${icon} ${escapeHTML(city.label)}</span>
				<span class="city-temp">${temp}</span>
				<span class="city-condition">${escapeHTML(desc)}</span>
			`;
			div.addEventListener('click', () => {
				elements.cityInput.value = city.label;
				getWeatherByCity(city.query);
			});
		} else {
			div.innerHTML = `
				<span class="city-name">${escapeHTML(city.label)}</span>
...
function schedulePopularCitiesRefresh({ force = false } = {}) {
	if (!force && popularCitiesCache) {
		renderPopularCities();
		return;
	}
	if (popularCitiesRefreshQueued) return;
	popularCitiesRefreshQueued = true;
	runAfterCriticalWork(() => {
		popularCitiesRefreshQueued = false;
		updatePopularCities({ force });
	}, 1800);
}

async function updatePopularCities({ force = false } = {}){
	if (!force && popularCitiesCache) {
		renderPopularCities();
		return;
	}
	try {
		const promises = POPULAR_CITIES.map(city => {
			return fetchWeather('weather', { q: city.query, units: 'metric' }, { silent: true }).catch(() => null);
		});

		const results = await Promise.all(promises);
		popularCitiesCache = POPULAR_CITIES.map((city, i) => ({ city, data: results[i] }));
		renderPopularCities();
	} catch(e) {
...
(function initSearchAutocomplete() {
	...
	function highlight(text, query) {
		...
		return (
			...
			`<span class="sd-match">${escapeHTML(text.slice(idx, idx + query.length))}</span>` +
	...
	function buildItem(cityName, regionLabel, query, weatherData) {
		...
		btn.id = `search-option-${currentItems.length}-${cityName.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`;
		btn.setAttribute('role', 'option');
		btn.setAttribute('aria-selected', 'false');
		btn.dataset.city = cityName;

		const icon = weatherData ? weatherIcon(weatherData.weather[0].main, 'weather-svg sd-svg') : uiIcon('globe', 'ui-icon', 'City');
		...
		btn.innerHTML = `
			<span class="sd-item-icon">${icon}</span>
			<span class="sd-item-name">${highlight(cityName, query)}</span>
			<span class="sd-item-region">${escapeHTML(regionLabel)}</span>
			<span class="sd-item-meta">
				${temp  ? `<span class="sd-item-temp">${temp}</span>` : ''}
				${cond  ? `<span class="sd-item-cond">${escapeHTML(cond)}</span>` : ''}
			</span>
		`;
	...
	function updateActiveItem(newIdx) {
		currentItems.forEach((el, i) => {
			...
			el.classList.toggle('sd-active', isActive);
			el.setAttribute('aria-selected', String(isActive));
		});
		if (currentItems[newIdx]) {
			input.setAttribute('aria-activedescendant', currentItems[newIdx].id);
...
elements.geoBtn.addEventListener('click', () => {
	if(!navigator.geolocation) {
		showWeatherStatus({
			...
			message: 'This browser does not support geolocation. Search for a city instead.'
		});
		return;
	}
	navigator.geolocation.getCurrentPosition(pos => {
		getWeatherByCoords(pos.coords.latitude, pos.coords.longitude);
	}, err => showWeatherStatus({
		title: 'Location permission failed',
		message: err.message || 'Search for a city instead.'
	}));
});

// Handle chart resize
let resizeChartFrame = 0;
window.addEventListener('resize', () => {
	if (hourlyData.length === 0) return;
	if (resizeChartFrame) cancelAnimationFrame(resizeChartFrame);
	resizeChartFrame = requestAnimationFrame(() => {
		resizeChartFrame = 0;
...
function updateWeatherBackground(weatherData) {
	...
	setTimeout(() => {
		...
		if (night) {
			...
			spawnRaindrops(bg, 38, true);
		} else if (cond.includes('rain') || cond.includes('drizzle')) {
			bg.classList.add('bg-rain');
			spawnRaindrops(bg, 24, false);
			...
			for (let i = 0; i < 2; i++) {
				...
				cloud.style.cssText = [
					...
					`animation-duration:${(58 + Math.random() * 38).toFixed(0)}s`
				].join(';');
			...
			for (let i = 0; i < 2; i++) {
				...
				cloud.style.cssText = [
					...
					`animation-duration:${(66 + Math.random() * 36).toFixed(0)}s`,
					`opacity:0.03`
				].join(';');
				bg.appendChild(cloud);
			}
...
function spawnRaindrops(container, count, heavy) {
	for (let i = 0; i < count; i++) {
		...
		const h = heavy ? (Math.random() * 11 + 9) : (Math.random() * 8 + 7);
		drop.style.cssText = [
			`left:${Math.random() * 100}%`,
			`height:${h.toFixed(0)}px`,
			`animation-delay:${(Math.random() * 2).toFixed(2)}s`,
			`animation-duration:${(Math.random() * 0.7 + (heavy ? 0.75 : 0.95)).toFixed(2)}s`,
			`opacity:${(Math.random() * 0.16 + 0.12).toFixed(2)}`
		].join(';');
...
function getOutfitSuggestion(weatherData) {
	...
	if (cond.includes('thunder') || cond.includes('storm')) {
		return {
			...
			activities: ['Stay home', 'Gaming', 'Cook at home', 'Movie night'],
	...
	if (cond.includes('rain')) {
		return {
			...
			activities: ['Coffee shop', 'Cinema', 'Read indoors', 'Mall day'],
	...
	if (cond.includes('drizzle')) {
		return {
			...
			activities: ['Cafe hopping', 'Museum', 'Mall', 'Creative work'],
	...
	if (cond.includes('snow')) {
		return {
			...
			activities: ['Snow activities', 'Cozy indoors', 'Hot drinks', 'Snow photos'],
	...
	if (effectiveTemp >= 37) {
		return {
			...
			activities: ['Pool or beach', 'Air-conditioned mall', 'Drinks run', 'Stay cool inside'],
	...
	if (effectiveTemp >= 33) {
		return {
			...
			activities: ['Beach day', 'Morning ride', 'Outdoor cafe', 'Photography'],
	...
	if (effectiveTemp >= 20) {
		return {
			...
			activities: ['Cafe work session', 'Outdoor yoga', 'Art visit', 'Walking tour'],
...
function renderOutfitPanel(weatherData) {
	...
	panel.innerHTML = `
		<div class="outfit-hero">
			<div class="outfit-icon-large">${uiIcon(outfit.icon, 'ui-icon', 'Outfit suggestion')}</div>
			<div>
		...
		<div class="outfit-activities">
			<div class="outfit-activities-label">${uiIcon('target', 'ui-icon', 'Activities')} Best Activities Today</div>
			<div class="activity-chips">
				${outfit.activities.map(a => `<span class="activity-chip">${a}</span>`).join('')}
			</div>
		</div>
		<div class="outfit-tip-bar">
			<span class="tip-icon">${uiIcon('info', 'ui-icon', 'Tip')}</span>
...
function shareWeatherCard() {
	if (!currentWeatherData) {
		showNavToast('Load weather data first');
		return;
	}
	...
	// Large weather mark
	drawWeatherMark(ctx, d.weather[0].main, 28, 34, 86);
	...
	ctx.fillStyle = 'rgba(255,255,255,0.5)';
	ctx.font = '13px -apple-system, BlinkMacSystemFont, sans-serif';
	ctx.fillText(`Humidity ${d.main.humidity}%   Wind ${Math.round(d.wind.speed)} m/s`, W - 30, 185);
...
function renderFeelLikeChart(list) {
	...
	humidity.forEach((h, i) => {
		...
		ctx.fillStyle = 'rgba(134,182,232,0.10)';
		ctx.beginPath();
		ctx.roundRect(x, y, barW, barH, 3);
		ctx.fill();
	});
	// Humidity axis labels (right)
	ctx.fillStyle = 'rgba(134,182,232,0.48)';
	...
	entries.forEach((e, i) => {
		...
		if (diff !== 0) {
			...
			ctx.fillStyle = diff > 0 ? 'rgba(249,115,22,0.7)' : 'rgba(134,182,232,0.62)';
	...
	canvas.setAttribute(
		'aria-label',
		`Feels-like temperature chart for the next 24 hours. Actual range ${formatTemp(Math.min(...temps))} to ${formatTemp(Math.max(...temps))}; feels-like range ${formatTemp(Math.min(...feelsLike))} to ${formatTemp(Math.max(...feelsLike))}.`
	);
}
...
function _getDismissed() {
	try { return new Set(JSON.parse(readStorage(sessionStorage, DISMISSED_ALERTS_KEY, '[]'))); }
	catch { return new Set(); }
}

function _saveDismissed(set) {
	writeStorage(sessionStorage, DISMISSED_ALERTS_KEY, JSON.stringify([...set]));
}
...
function generatePlainAlerts(weatherData, forecastList) {
	...
	if (cond.includes('thunder') || cond.includes('storm')) {
		candidates.push({ key: 'thunderstorm', severity: 'danger', icon: 'storm',
	...
	if (cond.includes('rain') && (desc.includes('heavy') || desc.includes('extreme'))) {
		candidates.push({ key: 'heavy-rain', severity: 'danger', icon: 'rain',
	...
	if (cond.includes('drizzle') || (cond.includes('rain') && desc.includes('light'))) {
		candidates.push({ key: 'light-rain', severity: 'info', icon: 'umbrella',
	...
	if (wind > 14) {
		candidates.push({ key: 'strong-wind', severity: 'danger', icon: 'wind',
			title: 'Strong Wind Warning',
			text: `Winds at ${Math.round(wind)} m/s — secure loose items outside. Avoid parking under trees or old structures.` });
	} else if (wind > 9) {
		candidates.push({ key: 'breezy', severity: 'warning', icon: 'wind',
	...
	if (feelsLk >= 40) {
		candidates.push({ key: 'extreme-heat', severity: 'danger', icon: 'warning',
			title: 'Extreme Heat Advisory',
			text: `Feels like ${formatTemp(feelsLk)} — dangerous heat index. Avoid going out 10AM–4PM. Drink water every 20 minutes.` });
	} else if (feelsLk >= 35) {
		candidates.push({ key: 'hot-day', severity: 'warning', icon: 'sun',
			title: 'Hot Day Ahead',
	...
	if (!cond.includes('rain') && !cond.includes('storm') && forecastList.length > 0) {
		...
		if (rainSoon) {
			candidates.push({ key: 'rain-later', severity: 'info', icon: 'drizzle',
...
function renderAlertsBanner(alerts, city, quiet = false) {
	...
	const rows = alerts.map((a, i) => `
		<div class="alert-item severity-${a.severity}${quiet ? ' alert-no-anim' : ''}"
			...
			<span class="alert-icon">${uiIcon(a.icon, 'ui-icon', a.title)}</span>
			<div class="alert-body">
				<div class="alert-title">${a.title}</div>
				<div class="alert-text">${a.text}</div>
			</div>
			<button type="button" class="alert-dismiss" title="Dismiss" aria-label="Dismiss alert">${uiIcon('x', 'ui-icon')}</button>
		</div>
	`).join('');
	...
	const toolbar = alerts.length >= 2
		? `<div class="alert-toolbar">
			<button type="button" class="alerts-dismiss-all" data-dismiss-all>
				Dismiss all for ${escapeHTML(city)}
			</button>
		   </div>`
		: '';
...
async function getWeatherByCity(city) {
	...
	try {
		...
		schedulePopularCitiesRefresh();
		fetchUVIndex(cur.coord.lat, cur.coord.lon).then(uv => {
			if(requestId === activeWeatherRequestId) renderUVIndex(uv);
		});
		clearWeatherStatus();
	} catch (err) {
		if(requestId !== activeWeatherRequestId) return;
		showWeatherError(err, () => getWeatherByCity(city));
		elements.temp.textContent = '--';
		elements.desc.textContent = '--';
		setAISummary('AI summary unavailable.', false);
	} finally {
		if(requestId === activeWeatherRequestId) clearLoading();
	}
}
...
async function getWeatherByCoords(lat, lon) {
	...
	try {
		const [cur, f] = await Promise.all([
			fetchWeather('weather', { lat, lon, units: 'metric' }),
			fetchWeather('forecast', { lat, lon, units: 'metric' })
		]);
		if(requestId !== activeWeatherRequestId) return;
		currentWeatherData = cur;
		forecastList = f.list;
		...
		generateAISummary(cur, f.list);
		schedulePopularCitiesRefresh();
		fetchUVIndex(lat, lon).then(uv => {
			if(requestId === activeWeatherRequestId) renderUVIndex(uv);
		});
		clearWeatherStatus();
	} catch (err) {
		if(requestId !== activeWeatherRequestId) return;
		showWeatherError(err, () => getWeatherByCoords(lat, lon));
		setAISummary('AI summary unavailable.', false);
	} finally {
		if(requestId === activeWeatherRequestId) clearLoading();
	}
...
function getFocusableElements(container) {
	...
	].join(','))].filter(el => !el.hasAttribute('hidden') && el.offsetParent !== null);
}
...
function openManagedModal(modal, focusTarget) {
	if(!modal) return;
	const opener = document.activeElement instanceof HTMLElement ? document.activeElement : null;
	modalState.set(modal, { opener });
	modal.style.display = 'flex';
	modal.removeAttribute('aria-hidden');
	document.body.style.overflow = 'hidden';
	const firstFocusable = getFocusableElements(modal)[0];
	(focusTarget || firstFocusable || modal).focus({ preventScroll: true });
	setAppInert(true);
}
...
function closeManagedModal(modal) {
	...
	const anyOpen = [...modalState.keys()].some(openModal => openModal.style.display !== 'none');
	if(!anyOpen) {
...
function getActiveModal() {
	return [...modalState.keys()].reverse().find(modal => modal.style.display !== 'none') || null;
}
...
document.addEventListener('keydown', event => {
	...
	if(event.key === 'Escape') {
		event.preventDefault();
		if(modal === compareModal) closeCompareModal();
		if(modal === citiesModal) closeCitiesModal();
		return;
	}
...
function openCompareModal() {
	openManagedModal(compareModal, compareCityInput);
	renderCompareGrid();
}

function closeCompareModal() {
	closeManagedModal(compareModal);
}
...
async function addCompareCity() {
	...
	if (pinnedCities.length >= 3) {
		showNavToast('Max 3 cities. Remove one first');
		return;
	}
	...
	tempDiv.id = tempId;
	tempDiv.innerHTML = `<div class="compare-card-loading">Loading ${escapeHTML(city)}...</div>`;
	if (compareGrid.querySelector('.compare-empty')) compareGrid.innerHTML = '';
	compareGrid.appendChild(tempDiv);

	try {
		const data = await fetchWeather('weather', { q: city, units: 'metric' });
		pinnedCities.push(data);
		document.getElementById(tempId)?.remove();
		renderCompareGrid();
	} catch (err) {
		document.getElementById(tempId)?.remove();
		if(err instanceof WeatherRequestError && err.kind === 'config') {
...
function scoreLabel(score){
	...
	if(score >= 20) return { text:'Stay covered',  color:'#f97316' };
...
function renderCompareGrid() {
	...
	pinnedCities.forEach((data, i) => {
		...
		const icon    = weatherIcon(data.weather[0].main, 'weather-svg compare-svg');
		const temp    = formatTemp(data.main.temp);
		...
		if (isCool) card.style.borderColor = 'rgba(134,182,232,0.24)';

		card.innerHTML = `
			<button type="button" class="compare-card-remove" data-remove-index="${i}" title="Remove" aria-label="Remove ${escapeHTML(data.name)}">${uiIcon('x', 'ui-icon')}</button>
			<div class="compare-card-top">
				<div>
					<div class="compare-card-city">${escapeHTML(data.name)}</div>
					<div class="compare-card-country">${escapeHTML(data.sys.country)}</div>
				</div>
				<div class="compare-card-icon">${icon}</div>
			</div>
			<div class="compare-card-temp">${temp}</div>
			<div class="compare-card-desc">${escapeHTML(data.weather[0].description)}</div>
			<div class="compare-card-stats">
				<div class="compare-stat">
					<div class="compare-stat-label">${uiIcon('thermometer', 'ui-icon', 'Feels like')} Feels Like</div>
					<div class="compare-stat-value">${feels}</div>
				</div>
				<div class="compare-stat">
					<div class="compare-stat-label">${uiIcon('droplet', 'ui-icon', 'Humidity')} Humidity</div>
					<div class="compare-stat-value">${data.main.humidity}%</div>
				</div>
				<div class="compare-stat">
					<div class="compare-stat-label">${uiIcon('wind', 'ui-icon', 'Wind')} Wind</div>
					<div class="compare-stat-value">${Math.round(data.wind.speed)} m/s</div>
				</div>
				<div class="compare-stat">
					<div class="compare-stat-label">${uiIcon('eye', 'ui-icon', 'Visibility')} Visibility</div>
			...
			<div class="compare-outdoor-score">
				<div class="outdoor-score-bar-track">
					<div class="outdoor-score-bar-fill" style="transform:scaleX(${score / 100});background:${scoreColor}"></div>
				</div>
			...
			${isBest ? '<div class="compare-temp-note compare-temp-note--hot">Hottest</div>' : ''}
			${isCool ? '<div class="compare-temp-note compare-temp-note--cool">Coolest</div>' : ''}
		`;
		card.querySelector('[data-remove-index]')?.addEventListener('click', () => removeCompareCity(i));
...
function renderSunpathArc(cur, sunriseStr, sunsetStr) {
	...
	ctx.textAlign = 'left';
	ctx.fillText('Sunrise ' + sunriseStr, cx - rx - 8, cy + 16);

	// Sunset label (right)
	ctx.textAlign = 'right';
	ctx.fillText(sunsetStr + ' Sunset', cx + rx + 8, cy + 16);
...
(function initPullToRefresh() {
	...
	indicator.innerHTML = `<span class="ptr-icon">${uiIcon('refresh', 'ui-icon', 'Refresh')}</span><span class="ptr-text">Pull to refresh</span>`;
...
const CITY_WEATHER_CONCURRENCY = 6;
let cityWeatherQueue = [];
let activeCityWeatherFetches = 0;
let cityWeatherQueueToken = 0;
let citiesModalFilterFrame = 0;
...
citiesModalSearch?.addEventListener('input', () => {
  if (citiesModalFilterFrame) cancelAnimationFrame(citiesModalFilterFrame);
  citiesModalFilterFrame = requestAnimationFrame(() => {
    citiesModalFilterFrame = 0;
...
function openCitiesModal() {
  ...
  buildCitiesModalDOM();
  openManagedModal(citiesModal, citiesModalSearch);
}

function closeCitiesModal() {
  citiesModalOpen = false;
  resetCityWeatherQueue();
  closeManagedModal(citiesModal);
}

function resetCityWeatherQueue() {
  cityWeatherQueue = [];
  cityWeatherQueueToken++;
}

function queueCityWeatherFetch(city, btnEl) {
  if (!citiesModalOpen || citiesWeatherCache[city]) return;
  cityWeatherQueue.push({ city, btnEl, token: cityWeatherQueueToken });
  runCityWeatherQueue();
}

function runCityWeatherQueue() {
  while (citiesModalOpen && activeCityWeatherFetches < CITY_WEATHER_CONCURRENCY && cityWeatherQueue.length > 0) {
    const job = cityWeatherQueue.shift();
    if (!job || job.token !== cityWeatherQueueToken) continue;
    activeCityWeatherFetches++;
    fetchCityWeather(job.city, job.btnEl)
      .finally(() => {
        activeCityWeatherFetches = Math.max(0, activeCityWeatherFetches - 1);
        runCityWeatherQueue();
      });
  }
}
...
function buildCitiesModalDOM() {
  ...
  Object.entries(CITIES_BY_REGION).forEach(([region, cities]) => {
    ...
    block.innerHTML = `<div class="cities-region-label">${escapeHTML(region)}</div>`;
    ...
    cities.forEach(city => {
      ...
      if (cached) {
        ...
        btn.innerHTML = `
          <span class="city-modal-icon">${uiIcon('globe', 'ui-icon', 'City')}</span>
          <div class="city-modal-info">
            <div class="city-modal-name">${escapeHTML(city)}</div>
            <div class="city-modal-meta">Loading...</div>
          </div>
          <span class="city-modal-temp loading-temp">...</span>
        `;
        queueCityWeatherFetch(city, btn);
      }
...
function buildCityCardHTML(data) {
  const icon = weatherIcon(data.weather[0].main, 'weather-svg city-modal-svg');
  const temp  = formatTemp(data.main.temp);
  const desc  = data.weather[0].description.replace(/\b\w/g, c => c.toUpperCase());
  return `
    <span class="city-modal-icon">${icon}</span>
    <div class="city-modal-info">
      <div class="city-modal-name">${escapeHTML(data.name)}</div>
      <div class="city-modal-meta">${escapeHTML(desc)}</div>
    </div>
...
async function fetchCityWeather(city, btnEl) {
  try {
    const data = await fetchWeather('weather', { q: `${city},PH`, units: 'metric' }, { silent: true });
    citiesWeatherCache[city] = data;
    // Update the button if still in the DOM
    if (citiesModalOpen && btnEl && btnEl.isConnected) {
